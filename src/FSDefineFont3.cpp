...
namespace transform
{
   FSDefineFont3::FSDefineFont3(FSInputStream* aStream) :
   ...
   const char* FSDefineFont3::className() const
   {
      const static char _name[] = "FSDefineFont3";
      
      return _name;
   }
   
   void FSDefineFont3::add(const FSVector<FSShape>& anArray)
   {
      for (FSVector<FSShape>::const_iterator i = anArray.begin(); i != anArray.end(); ++i)
         shapes.push_back(*i);
   }
   
   void FSDefineFont3::addCode(const FSVector<int>& anArray)
   {
      for (FSVector<int>::const_iterator i = anArray.begin(); i != anArray.end(); ++i)
         codes.push_back(*i);
   }
   
   void FSDefineFont3::addAdvance(const FSVector<int>& anArray)
   {
      for (FSVector<int>::const_iterator i = anArray.begin(); i != anArray.end(); ++i)
         advances.push_back(*i);
   }
   
   void FSDefineFont3::add(const FSVector<FSBounds>& anArray)
   {
      for (FSVector<FSBounds>::const_iterator i = anArray.begin(); i != anArray.end(); ++i)
         bounds.push_back(*i);
   }
   
   void FSDefineFont3::add(const FSVector<FSKerning>& anArray)
   {
      for (FSVector<FSKerning>::const_iterator i = anArray.begin(); i != anArray.end(); ++i)
         kernings.push_back(*i);
   }
   
   int FSDefineFont3::lengthInStream(FSOutputStream* aStream)
   {
      int glyphLength = 0;
      int wideCodes = 0;
   ...
   void FSDefineFont3::encodeToStream(FSOutputStream* aStream)
   {
      int wideCodes = 0;
      int format = 0;
   ...
   void FSDefineFont3::decodeFromStream(FSInputStream* aStream)
   {
   ...
   bool FSDefineFont3::containsLayoutInfo() const
   {
      bool layout = false;
