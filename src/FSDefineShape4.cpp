...
namespace transform
{
   FSDefineShape4::FSDefineShape4(FSInputStream* aStream) :
   FSDefineObject(DefineShape4, 0),
   bounds(),
   fillStyles(),
   lineStyles(),
   shape(FSShape())
   ...
   FSDefineShape4::FSDefineShape4(const FSDefineShape4& rhs) :
   FSDefineObject(rhs),
   bounds(rhs.bounds),
   ...
   const char* FSDefineShape4::className() const
   {
      const static char _name[] = "FSDefineShape4";
      
      return _name;
   }
   
   FSDefineShape4& FSDefineShape4::operator= (const FSDefineShape4& rhs)
   {
      if (this != &rhs)
      {
   ...
   void FSDefineShape4::encodeToStream(FSOutputStream* aStream)
   {
      int numberOfFillBits = fillStyles.size() > 0 ? FSStream::FieldSize((int)(fillStyles.size())): 0;
      int numberOfLineBits = lineStyles.size() > 0 ? FSStream::FieldSize((int)(lineStyles.size())): 0;
   ...
   void FSDefineShape4::decodeFromStream(FSInputStream* aStream)
   {
      int fillStyleCount = 0;
      int lineStyleCount = 0;
      
      aStream->startDecoding(className());
      unsigned int cursor_start = aStream->getCursor();
      FSDefineObject::decodeFromStream(aStream);
      
      aStream->setContext(FSStream::ColorContainsAlpha, 1);
      
      bounds.decodeFromStream(aStream);
      edgeBounds.decodeFromStream(aStream);
      aStream->read(transform::FSStream::UnsignedBit, 5);// Reserved
      usesFillWindingRule = aStream->read(transform::FSStream::UnsignedBit, 1);
      usesNonScalingStrokes = aStream->read(transform::FSStream::UnsignedBit, 1);
      usesScalingStrokes = aStream->read(transform::FSStream::UnsignedBit, 1);
      ...
      // Crash on unknown type
      // Hack for skip end of this tag
      aStream->setCursor(cursor_start);
      FSMovieObject::decodeFromStream(aStream);
         //aStream->setCursor(aStream->getCursor() + getLength());
      unsigned int next = aStream->getCursor() + (length << 3);
